...
import com.byndl.avantimarket.ui.adapter.ExpiringSoonSpecialsAdapter;

public class ExpiringSoonSpecialsFragment extends BaseFragment {

	private ListView transactionsList = null;
	private ExpiringSoonSpecialsAdapter expiringSoonSpecialsAdapter = null;
	private View rootView = null;
	private HashMap<Integer, String> specialsMap = null;
	private List<Integer> sortedIndex = null;
	private Button fixedHeader = null;
	@Override
	public View onCreateView(LayoutInflater inflater,
			@Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
		rootView = inflater.inflate(R.layout.expiring_soon_specials_fragment, container, false);
	...
	private void initialiseClassMembers() {
		...
		fixedHeader = (Button) rootView.findViewById(R.id.m_expiringSpanFixedHeader);
		fixedHeader.setVisibility(View.GONE);
		transactionsList.setDivider(null);
		expiringSoonSpecialsAdapter = new ExpiringSoonSpecialsAdapter(parentActivity,
				R.layout.expiring_soon_specials_item, mALSpecials, specialsMap, fixedHeader);
		transactionsList.setAdapter(expiringSoonSpecialsAdapter);
	}
	...
	public void onResume() {
		...
		expiringSoonSpecialsAdapter.notifyDataSetChanged();
	}
	...
	private void removeOutOfScopeDates() {
		...
		for(int i=0; i< sortedIndex.size(); i++) {
			...
			if(offerEndDate.compareTo(todayDate) < 0 ||
					offerEndDate.compareTo(thirtiethDayDate) > 0) {
